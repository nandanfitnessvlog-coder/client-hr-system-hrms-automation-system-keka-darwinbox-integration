...
import { collection, getDocs, addDoc, query, orderBy, limit, doc, getDoc, setDoc, where, updateDoc, deleteDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.12.1/firebase-firestore.js";
import { sendNotification } from "./notification-app.js";

// ===== MOCK DATA FALLBACKS =====
const mockEmployees = [
    { id: 'emp4', name: 'Alice Cooper', role: 'Logistics Coord', dept: 'Operations' },
    { id: 'emp5', name: 'Bob Marley', role: 'Warehouse Mgr', dept: 'Operations' },
    { id: 'emp6', name: 'Charlie Sheen', role: 'Ops Analyst', dept: 'Operations' }
];

const mockProcesses = [
    { id: 'proc1', name: 'Q2 Fulfillment Cycle', type: 'Fulfillment', efficiency: 92, nextAudit: '2026-05-15', assigneeName: 'Alice Cooper' },
    { id: 'proc2', name: 'Raw Material QC', type: 'Quality Control', efficiency: 98, nextAudit: '2026-05-20', assigneeName: 'Bob Marley' }
];

const mockInventory = [
    { id: 'inv1', name: 'Packaging Material B', sku: 'SKU-001-B', stock: 8, assigneeName: 'Alice Cooper' },
    { id: 'inv2', name: 'Hydraulic Fluid XL', sku: 'SKU-099-XL', stock: 45, assigneeName: 'Bob Marley' }
];

const mockActivities = [
    { user: 'Alice Cooper', action: 'updated stock for Packaging Material B', time: '5 mins ago', color: 'blue', initials: 'AC' },
    { user: 'Bob Marley', action: 'completed QC audit for Unit-7', time: '1 hr ago', color: 'purple', initials: 'BM' }
];

const mockTasks = [
    { id: 't1', title: 'Approve PO #8842', assigneeName: 'Sarah Jenkins', priority: 'High', deadline: '2026-05-06', status: 'pending' },
    { id: 't2', title: 'Schedule Unit-7 Maintenance', assigneeName: 'Bob Marley', priority: 'Critical', deadline: '2026-05-07', status: 'in-progress' }
];
...
const pageTitles = {
  ...
  'ai-messaging': 'Process Optimizer',
  'bank': 'Operational Budget',
  'integration': 'ERP Integrations',
  'settings': 'Settings',
  'notifications': 'Operations Inbox'
};

// --- Inbox Logic ---
window.markAllRead = () => {
    document.querySelectorAll('.inbox-card').forEach(card => card.classList.remove('unread'));
    if (window.showToast) window.showToast('Inbox Updated', 'All notifications marked as read.', 'success');
};

window.selectNotif = (card, type) => {
    ...
    const contents = {
        'process': {
            ...
            from: 'System Monitor',
            time: '2 minutes ago',
            body: `<p>Hello Alex,</p><p>A critical bottleneck has been identified in the <b>"Q2 Fulfillment Cycle"</b> at the Warehouse-A station. Throughput has dropped by 40%.</p><p>Please review the resource allocation and consider diverting additional personnel to Station-4.</p><p>Best regards,<br>Ops AI</p>`
        },
        'inventory': {
            tag: 'Supply Chain',
            ...
            from: 'ERP Sync',
            time: '15 minutes ago',
            body: `<p>Inventory levels for "Packaging Material B" have fallen below the 10% safety threshold.</p><p><b>Action Required:</b> Approve the automated re-order request (PO #8842) to avoid production downtime.</p>`
        },
        'ai': {
            tag: 'AI Insights',
            ...
            time: '2 hours ago',
            body: `<p>The AI Engine suggests that batching "Order Type X" during night shifts could reduce energy costs by ₹15,000 per week.</p><p><b>Recommendation:</b> Reschedule the batching sequence starting next Monday.</p>`
        },
        'maintenance': {
            ...
            from: 'IoT Sensor',
            time: 'Yesterday',
            body: `<p>Sensor data from Unit-7 indicates abnormal vibration patterns. Predicted failure within 48 hours.</p><p>Scheduled maintenance recommended immediately to prevent unplanned shutdown.</p>`
        }
    };

    const data = contents[type] || contents['campaign'];
    
    detail.innerHTML = `
        <div class="detail-header">
            <span class="detail-tag tag-${type}">${data.tag}</span>
            <h2 class="detail-title">${data.title}</h2>
            <div class="detail-meta">
                <span style="display:flex; align-items:center; gap:6px;"><i data-lucide="user" size="14"></i> From: ${data.from}</span>
                <span style="display:flex; align-items:center; gap:6px;"><i data-lucide="clock" size="14"></i> ${data.time}</span>
            </div>
        </div>
        <div class="detail-content">${data.body}</div>
        <div class="detail-actions">
            <button class="btn btn-primary" style="padding: 1rem 2rem;">Take Action</button>
            <button class="btn btn-outline" style="padding: 1rem 2rem;">Mark as Important</button>
        </div>
    `;
...
const showModal = (title, msg, icon = 'info', isConfirm = false, onConfirm = null) => {
  ...
  if (mIcon) {
    mIcon.innerHTML = `<i data-lucide="${icon}"></i>`;
    if (window.lucide) lucide.createIcons();
  }
  ...
  btnCancel.onclick = closeLogoutModal;

  if (modal) modal.classList.add('active');
};

const handleLogout = () => {
  showModal('Logout Confirmation', 'Are you sure you want to end your operational session?', 'log-out', true, async () => {
    try {
      await signOut(auth);
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    try {
      ...
      console.warn('Operations fetch failed, using fallback.');
    }

    if (!data) {
      data = {
        name: localStorage.getItem('userName') || user.displayName || user.email.split('@')[0],
        role: localStorage.getItem('userRole') || 'Operations Manager',
        departmentId: localStorage.getItem('userDept') || 'operations'
      };
    }
    ...
    if (welcomeGreeting) welcomeGreeting.innerHTML = `Good Day, ${data.name.split(' ')[0]} 👋`;
...
function updateCharts() {
  const campCtx = document.getElementById('campaignChart');
  const chanCtx = document.getElementById('channelChart');

  if (!campCtx || !chanCtx) return;
  ...
  channelChartInstance = new Chart(chanCtx, {
    ...
    data: {
      ...
      datasets: [{ 
        data: [45, 25, 15, 10, 5], 
        backgroundColor: ['#8b5cf6','#d946ef','#3b82f6','#10b981','#f59e0b'], 
        borderWidth: 0, 
        hoverOffset: 6 
      }]
    },
...
async function loadBankDetails() {
  try {
    if (document.getElementById('uiBankAcc')) document.getElementById('uiBankAcc').textContent = '**** 8842';
    if (document.getElementById('uiBankName')) document.getElementById('uiBankName').textContent = 'Axis Bank';
    if (document.getElementById('uiBankRouting')) document.getElementById('uiBankRouting').textContent = 'Routing: 099000099';
    if (document.getElementById('uiBankBalance')) document.getElementById('uiBankBalance').textContent = '₹1,450,000.00';
  } catch (e) {
...
async function loadDisbursements() {
  ...
  try {
      const mockPayroll = [
          { name: 'Alice Cooper', month: 'April 2026', processedAt: new Date().toISOString(), net: '65,000' },
          { name: 'Bob Marley', month: 'April 2026', processedAt: new Date().toISOString(), net: '55,000' }
      ];
      list.innerHTML = mockPayroll.map(p => {
        const initial = p.name?.[0] || 'P';
        return `
          <div class="disbursement-item">
            <div class="act-avatar blue">${initial}</div>
            <div class="act-body"><b>${p.name}</b> ${p.month}<span class="act-time">${new Date(p.processedAt).toLocaleDateString()}</span></div>
            <span class="act-badge success">₹${p.net}</span>
          </div>
        `;
      }).join('');
  } catch (e) {
...
async function loadDashboardData() {
  try {
    ...
    try {
      const userDept = localStorage.getItem('userDept') || 'operations';
      ...
      if (response.ok) {
        ...
        const deptEmps = result.data.filter(e => e.departmentId === userDept || e.dept === userDept);
        if (deptEmps.length > 0) {
          renderTeamList(deptEmps);
          populateTeamDropdowns(deptEmps);
        } else {
          renderTeamList(mockEmployees);
          populateTeamDropdowns(mockEmployees);
        }
      } else {
        throw new Error('Backend fetch failed');
      }
    } catch (err) {
      console.warn('Backend sync failed, using Firestore/Mock fallback');
      if (auth.currentUser) {
        try {
          const snap = await getDocs(query(collection(db, 'users'), where('departmentId', 'in', ['operations', 'operational'])));
          const emps = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          const finalEmps = emps.length > 0 ? emps : mockEmployees;
          renderTeamList(finalEmps);
          populateTeamDropdowns(finalEmps);
        } catch (fsErr) {
...
// --- UI Utilities ---
function populateTeamDropdowns(employees) {
...
if (btnGenAI) {
    btnGenAI.addEventListener('click', () => {
        ...
        if (!prompt) {
            if (window.showToast) window.showToast('Input Required', 'Please enter some process context.', 'warning');
            return;
        }

        btnGenAI.disabled = true;
        btnGenAI.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Optimizing Process...';
        aiResult.style.display = 'block';
        aiResult.innerHTML = '<div class="typing-indicator"><span></span><span></span><span></span></div> AI is simulating process flows...';

        setTimeout(() => {
            const results = [
                "⚙️ <b>Workflow Optimization:</b><br>Identified redundant step in [Process Name]. By merging Stage 2 and 3, you can reduce total lead time by 15%. Implementing this change requires updating the SOP for the morning shift.",
                "📊 <b>Resource Allocation:</b><br>Current load on Unit [X] is 95% while Unit [Y] is at 40%. Redistributing tasks will prevent burnout and reduce the risk of mechanical failure by 22%.",
                "🛡️ <b>Quality Control:</b><br>Based on recent error logs, the AI suggests increasing the sampling frequency for the [Specific Part] batch. This will catch defects 3x earlier than the current weekly audit."
            ];
            const random = results[Math.floor(Math.random() * results.length)];
            aiResult.innerHTML = `<div style="color:var(--primary); margin-bottom:0.5rem; font-weight:700;"><i class="fa-solid fa-robot"></i> AI PROCESS OPTIMIZATION:</div>${random}`;
            btnGenAI.disabled = false;
            btnGenAI.innerHTML = 'Re-run Optimization';
            if (window.showToast) window.showToast('Optimization Complete', 'AI has successfully generated a process improvement plan.', 'success');
        }, 2500);
    });
}
...
async function loadProcesses() {
    ...
    if (!tbody) return;
    try {
        tbody.innerHTML = mockProcesses.map(c => `
            <tr>
                <td><b>${c.name}</b></td>
                <td><span class="status-pill" style="background:#eff6ff; color:#3b82f6;">${c.type}</span></td>
                <td>${c.efficiency || '95%'}%</td>
                <td>${c.nextAudit || 'No Date'}</td>
                <td>${c.assigneeName || 'Unassigned'}</td>
                <td><span class="status-pill pill-present">Running</span></td>
                <td>
                    <button class="action-btn" title="Edit"><i data-lucide="edit" size="14"></i></button>
                </td>
            </tr>
        `).join('');
        if (window.lucide) lucide.createIcons();
...
async function loadInventory() {
    ...
    if (!tbody) return;
    try {
        tbody.innerHTML = mockInventory.map(l => `
            <tr>
                <td><b>${l.name}</b></td>
                <td>${l.sku}</td>
                <td>${l.assigneeName || 'Unassigned'}</td>
                <td><span class="status-pill ${l.stock < 10 ? 'pill-late' : 'pill-present'}">${l.stock < 10 ? 'Low' : 'OK'}</span></td>
                <td>
                    <button class="action-btn" title="Order"><i data-lucide="shopping-cart" size="14"></i></button>
                </td>
            </tr>
        `).join('');
        if (window.lucide) lucide.createIcons();
...
// --- Modal Controls ---
window.openCreateProcessModal = () => { document.getElementById('createProcessModal').style.display = 'flex'; };
window.closeCreateProcessModal = () => { document.getElementById('createProcessModal').style.display = 'none'; };
window.openCreateInventoryModal = () => { document.getElementById('createInventoryModal').style.display = 'flex'; };
window.closeCreateInventoryModal = () => { document.getElementById('createInventoryModal').style.display = 'none'; };
window.openCreateTaskModal = () => { document.getElementById('createTaskModal').style.display = 'flex'; };
window.closeCreateTaskModal = () => { document.getElementById('createTaskModal').style.display = 'none'; };
...
if (btnSaveProcess) {
    btnSaveProcess.addEventListener('click', async () => {
        ...
        const efficiency = document.getElementById('procEff').value;
        const nextAudit = document.getElementById('procAudit').value;
        const assigneeId = document.getElementById('procAssignee').value;
        const assigneeName = document.getElementById('procAssignee').options[document.getElementById('procAssignee').selectedIndex]?.text || '';

        if (!name || !efficiency) {
            if (window.showToast) window.showToast('Error', 'Please fill in required fields.', 'error');
            return;
        }

        try {
            btnSaveProcess.disabled = true;
            btnSaveProcess.textContent = 'Saving...';
            // Mock Save
            mockProcesses.push({ id: 'proc' + Date.now(), name, type, efficiency, nextAudit, assigneeId, assigneeName });
            closeCreateProcessModal();
            loadProcesses();
            if (window.showToast) window.showToast('Process Saved', 'New operational process has been recorded.', 'success');
        } catch (e) { console.error(e); } finally {
            btnSaveProcess.disabled = false;
            btnSaveProcess.textContent = 'Save';
        }
    });
}
...
if (btnSaveInventory) {
    btnSaveInventory.addEventListener('click', async () => {
        ...
        const assigneeId = document.getElementById('invAssignee').value;
        const assigneeName = document.getElementById('invAssignee').options[document.getElementById('invAssignee').selectedIndex]?.text || '';

        if (!name || !sku) return;

        try {
            btnSaveInventory.disabled = true;
            btnSaveInventory.textContent = 'Saving...';
            // Mock Save
            mockInventory.push({ id: 'inv' + Date.now(), name, sku, stock, assigneeId, assigneeName });
            closeCreateInventoryModal();
            loadInventory();
            if (window.showToast) window.showToast('Inventory Added', 'New stock item recorded.', 'success');
        } catch (e) { console.error(e); } finally {
            btnSaveInventory.disabled = false;
            btnSaveInventory.textContent = 'Save';
        }
    });
}
...
async function loadLeaveRequests() {
  ...
  try {
    const mockLeaves = [
        { id: 'lv3', userName: 'Alice Cooper', userDept: 'Operations', reason: 'Family Event', startDate: '2026-05-12', endDate: '2026-05-14', status: 'pending' },
        { id: 'lv4', userName: 'Bob Marley', userDept: 'Operations', reason: 'Medical', startDate: '2026-05-01', endDate: '2026-05-02', status: 'approved' }
    ];
    tbody.innerHTML = mockLeaves.map(req => `
      <tr>
...
window.handleLeaveAction = async (requestId, action) => {
    if (window.showToast) window.showToast('Leave Updated', `Request has been ${action}. Mock notification sent.`, action === 'approved' ? 'success' : 'warning');
};
loadLeaveRequests();
...
if (btnCloseModal) btnCloseModal.onclick = closeOverlayModal;
if (btnCancelModal) btnCancelModal.onclick = closeOverlayModal;
if (modalOverlay) {
  modalOverlay.onclick = (e) => { if (e.target === modalOverlay) closeOverlayModal(); };
}
...
async function seedDatabase() {
  ...
  showToast('Initializing System...', 'Seeding all departments, managers, and employees...', 'info');
  ...
  try {
    ...
    const departments = [
      ...
      { departmentId: 'hr', departmentName: 'Human Resources', departmentCode: 'HRD' },
      { departmentId: 'engineering', departmentName: 'Engineering', departmentCode: 'ENG' }
    ];
    for (const d of departments) await setDoc(doc(db, 'departments', d.departmentId), d);

    // 2. Users (Managers & Employees)
    const users = [
      { name: 'System Admin', email: 'admin@hrflow.com', role: 'admin', departmentId: 'All' },
      { name: 'Marketing Manager', email: 'marketing.mgr@hrflow.com', role: 'manager', departmentId: 'Marketing' },
      { name: 'Finance Manager', email: 'finance.mgr@hrflow.com', role: 'manager', departmentId: 'Finance' },
      { name: 'HR Manager', email: 'hr.mgr@hrflow.com', role: 'manager', departmentId: 'HR' },
      { name: 'Engineering Manager', email: 'engineering.mgr@hrflow.com', role: 'manager', departmentId: 'Engineering' },
      { name: 'Marketing Emp1', email: 'marketing.emp1@hrflow.com', role: 'employee', departmentId: 'Marketing' },
      { name: 'Marketing Emp2', email: 'marketing.emp2@hrflow.com', role: 'employee', departmentId: 'Marketing' },
      { name: 'Finance Emp1', email: 'finance.emp1@hrflow.com', role: 'employee', departmentId: 'Finance' },
      { name: 'Finance Emp2', email: 'finance.emp2@hrflow.com', role: 'employee', departmentId: 'Finance' },
      { name: 'HR Emp1', email: 'hr.emp1@hrflow.com', role: 'employee', departmentId: 'HR' },
      { name: 'HR Emp2', email: 'hr.emp2@hrflow.com', role: 'employee', departmentId: 'HR' },
      { name: 'Engineering Emp1', email: 'engineering.emp1@hrflow.com', role: 'employee', departmentId: 'Engineering' },
      { name: 'Engineering Emp2', email: 'engineering.emp2@hrflow.com', role: 'employee', departmentId: 'Engineering' }
    ];
    for (const u of users) {
      // Note: This seeds Firestore. Auth accounts must still be created via sign-up or console.
      await addDoc(collection(db, 'users'), { ...u, createdAt: serverTimestamp() });
      await addDoc(collection(db, 'employees'), { ...u, status: 'active', color: '#3b82f6', initials: u.name.split(' ').map(n => n[0]).join('') });
    }

    // 3. Initial Payroll Data
    for (const p of payrollData) {
      await addDoc(collection(db, 'payroll'), { ...p, processedAt: serverTimestamp() });
    }

    // 4. Bank Details
    await setDoc(doc(db, 'bank_details', 'main'), { 
      account: '•••• •••• •••• 8842', 
    ...
    showToast('Success!', 'System database initialized with all managers and employees.', 'success');
...
async function loadTasks() {
    ...
    try {
        ...
        console.warn('Error loading tasks from Firestore, using mock fallback:', err);
        managerTaskList.innerHTML = mockTasks.map(task => `
            <tr>
                <td>
                    <div style="font-weight:600;">${task.title}</div>
                    <div style="font-size:0.75rem; color:var(--text-muted);">${task.description || 'Standard Operational Task'}</div>
                </td>
                <td>${task.assigneeName || 'Unassigned'}</td>
                <td>${task.deadline || 'No Deadline'}</td>
                <td><span class="status-pill ${typeof getStatusClass === 'function' ? getStatusClass(task.status) : 'pill-pending'}">${task.status}</span></td>
                <td>
                    <button class="action-btn" title="Delete Task" style="border:none; background:none; cursor:pointer; color:var(--danger);"><i data-lucide="trash-2" size="14"></i></button>
                </td>
            </tr>
        `).join('');
        if (window.lucide) lucide.createIcons();
...
// --- Real-Time Messaging ---
const btnSendRealMessage = document.getElementById('btnSendRealMessage');
if (btnSendRealMessage) {
    btnSendRealMessage.addEventListener('click', async () => {
        const recipientId = document.getElementById('msgRecipient').value;
        const subject = document.getElementById('msgSubject').value;
        const body = document.getElementById('msgBody').value;
        const senderId = auth.currentUser ? auth.currentUser.uid : localStorage.getItem('hr_user_id');

        if (!recipientId || !body) {
            if (window.showToast) window.showToast('Error', 'Please select a recipient and enter a message.', 'error');
            return;
        }

        try {
            btnSendRealMessage.disabled = true;
            btnSendRealMessage.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Sending...';
            ...
            await sendNotification(recipientId, 'New Direct Message', `Subject: ${subject || 'No Subject'}`, 'message');
            
            if (window.closeSendMessageModal) window.closeSendMessageModal();
            document.getElementById('msgSubject').value = '';
            document.getElementById('msgBody').value = '';
            
            if (window.showToast) window.showToast('Message Sent', 'Your message was delivered successfully.', 'success');
        } catch (err) {
            console.error('Messaging error:', err);
            if (window.showToast) window.showToast('Failed to Send', 'Could not deliver message. Please try again.', 'error');
        } finally {
            btnSendRealMessage.disabled = false;
            btnSendRealMessage.textContent = 'Send Message';
        }
    });
}
