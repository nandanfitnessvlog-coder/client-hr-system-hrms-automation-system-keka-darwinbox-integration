...
// ===== MOCK DATA FALLBACKS =====
const mockEmployees = [
    { id: 'emp7', name: 'Sarah Rose', role: 'Creative Director', dept: 'Marketing' },
    { id: 'emp8', name: 'John Mayer', role: 'SEO Specialist', dept: 'Marketing' },
    { id: 'emp9', name: 'Katy Perry', role: 'Social Media Mgr', dept: 'Marketing' }
];

let mockCampaigns = [
    { id: 'c1', name: 'Autumn Fashion Gala 2026', platform: 'Instagram', budget: '500,000', endDate: '2026-05-30', assigneeName: 'Sarah Rose' },
    { id: 'c2', name: 'Tech Expo 2026 Sponsorship', platform: 'LinkedIn', budget: '1,200,000', endDate: '2026-06-15', assigneeName: 'John Mayer' },
    { id: 'c3', name: 'Winter Collection Teaser', platform: 'TikTok', budget: '250,000', endDate: '2026-07-01', assigneeName: 'Katy Perry' }
];

let mockLeads = [
    { id: 'l1', name: 'Robert Fox', email: 'robert@foxtech.com', assigneeName: 'Sarah Rose', status: 'Warm' },
    { id: 'l2', name: 'Sarah Connor', email: 'sarah@cyberdyne.io', assigneeName: 'John Mayer', status: 'Hot' },
    { id: 'l3', name: 'Bruce Wayne', email: 'bruce@waynecorp.com', assigneeName: 'Katy Perry', status: 'Warm' }
];
...
const pageTitles = {
  ...
  'notifications': 'Marketing Inbox',
  'ai-messaging': 'AI Ad Copy Generator',
  'bank': 'Marketing Budget',
  'integration': 'Ad Integrations',
  'settings': 'Settings'
};
...
window.selectNotif = (card, type) => {
    ...
    const contents = {
        'campaign': {
            ...
            from: 'Sarah Rose',
            time: '2 minutes ago',
            body: `<p>Hello Alex,</p><p>I've assigned you as the lead manager for the <b>"Autumn Fashion Gala 2026"</b> campaign. This is one of our flagship events for the Q3 marketing cycle.</p><p>Please review the initial brief and budget allocation in the Campaigns tab. We need to finalize the platform strategy by the end of this week.</p><p>Best regards,<br>Sarah Rose</p>`
        },
        'lead': {
            tag: 'Lead Generation',
            ...
            from: 'AI Engine',
            time: '15 minutes ago',
            body: `<p>A potential lead from New York has shown 92% purchase intent based on social signals and recent interactions with our Autumn campaign landing page.</p><p><b>Action Required:</b> Initiate personalized outreach within the next 4 hours to capitalize on this high-intent window.</p>`
        },
        'ai': {
            tag: 'AI Insights',
            ...
            time: '2 hours ago',
            body: `<p>The AI Engine detected a 15% drop in engagement for Instagram story placements over the last 48 hours.</p><p><b>Recommendation:</b> Reallocate ₹50,000 from Instagram stories to Reels video placements where CTR is currently 3.2x higher.</p>`
        },
        'budget': {
            tag: 'Campaign Management',
            ...
            from: 'Finance Bot',
            time: 'Yesterday',
            body: `<p>Your "Autumn Gala" campaign budget on Google Ads has reached 90% of its monthly limit (₹450,000 / ₹500,000).</p><p>Please approve a budget extension or prepare to throttle bids to avoid campaign pause.</p>`
        }
    };
...
const handleLogout = () => {
  showModal('Logout Confirmation', 'Are you sure you want to end your campaign session?', 'log-out', true, async () => {
    try {
      await signOut(auth);
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    try {
      ...
      console.warn('Firestore fetch failed, using local fallback.');
    }

    if (!data) {
      data = {
        name: localStorage.getItem('userName') || user.displayName || user.email.split('@')[0],
        role: localStorage.getItem('userRole') || 'Marketing Manager',
        departmentId: localStorage.getItem('userDept') || 'marketing'
      };
    }
    ...
    sidebarRoles.forEach(r => r.textContent = data.role.charAt(0).toUpperCase() + data.role.slice(1));
...
async function loadBankDetails() {
  try {
    if (document.getElementById('uiBankAcc')) document.getElementById('uiBankAcc').textContent = '**** 1234';
    if (document.getElementById('uiBankName')) document.getElementById('uiBankName').textContent = 'ICICI Bank';
    if (document.getElementById('uiBankRouting')) document.getElementById('uiBankRouting').textContent = 'Routing: 055000055';
    if (document.getElementById('uiBankBalance')) document.getElementById('uiBankBalance').textContent = '₹950,000.00';
  } catch (e) {
...
async function loadDisbursements() {
  ...
  try {
      const mockPayroll = [
          { name: 'Sarah Rose', month: 'April 2026', processedAt: new Date().toISOString(), net: '95,000' },
          { name: 'John Mayer', month: 'April 2026', processedAt: new Date().toISOString(), net: '75,000' }
      ];
...
async function loadDashboardData() {
  try {
    ...
    try {
      const userDept = localStorage.getItem('userDept') || 'marketing';
      ...
      if (auth.currentUser) {
        try {
          const snap = await getDocs(query(collection(db, 'users'), where('departmentId', '==', 'marketing')));
...
if (btnGenAI) {
    btnGenAI.addEventListener('click', () => {
        ...
        if (!prompt) {
            if (window.showToast) window.showToast('Input Required', 'Please enter some campaign context.', 'warning');
            return;
        }
        ...
        aiResult.innerHTML = '<div class="typing-indicator"><span></span><span></span><span></span></div> AI is crafting your ad copy...';

        setTimeout(() => {
            const results = [
                "✨ <b>Autumn Elegance Awaits.</b><br>Discover the collection that's setting the trend for 2026. Hand-crafted pieces designed for the modern visionary. Shop now for exclusive early access. #AutumnGala",
                "🚀 <b>Elevate Your Style.</b><br>The Autumn Fashion Gala is more than an event—it's a movement. Be the first to witness the fusion of tradition and technology. Limited slots for the VIP preview available now.",
                "💎 <b>Luxury Redefined.</b><br>Experience the pinnacle of fashion craftsmanship. Our Autumn line is here to redefine your wardrobe. Premium fabrics, unparalleled design. Join the gala."
            ];
            const random = results[Math.floor(Math.random() * results.length)];
            aiResult.innerHTML = `<div style="color:var(--primary); margin-bottom:0.5rem; font-weight:700;"><i class="fa-solid fa-robot"></i> AI REFINED COPY:</div>${random}`;
            btnGenAI.disabled = false;
            btnGenAI.innerHTML = 'Regenerate Ad Copy';
            if (window.showToast) window.showToast('Copy Generated', 'AI has successfully refined your campaign copy.', 'success');
        }, 2500);
    });
}

// --- Campaign & Lead Loading ---
async function loadCampaigns() {
    const tbody = document.getElementById('campaignList');
    if (!tbody) return;
    try {
        tbody.innerHTML = mockCampaigns.map(c => `
            <tr>
                <td><b>${c.name}</b></td>
                <td><span class="status-pill" style="background:#eff6ff; color:#3b82f6;">${c.platform}</span></td>
                <td>₹${c.budget}</td>
                <td>${c.endDate || 'No Date'}</td>
                <td>${c.assigneeName || 'Unassigned'}</td>
                <td><span class="status-pill pill-present">Live</span></td>
                <td>
...
// --- Modal Controls ---
window.openCreateCampaignModal = () => { document.getElementById('createCampaignModal').style.display = 'flex'; };
window.closeCreateCampaignModal = () => { document.getElementById('createCampaignModal').style.display = 'none'; };
...
if (btnSaveCampaign) {
    btnSaveCampaign.addEventListener('click', async () => {
        ...
        const assigneeId = document.getElementById('campAssignee').value;
        const assigneeName = document.getElementById('campAssignee').options[document.getElementById('campAssignee').selectedIndex]?.text || '';
        ...
        try {
            ...
            btnSaveCampaign.textContent = 'Saving...';
            // Mock Save
            mockCampaigns.push({ id: 'c' + Date.now(), name, platform, budget, endDate, assigneeId, assigneeName });
            closeCreateCampaignModal();
            loadCampaigns();
            if (window.showToast) window.showToast('Campaign Saved', 'New campaign has been added to the tracking system.', 'success');
        } catch (e) { console.error(e); } finally {
            btnSaveCampaign.disabled = false;
            btnSaveCampaign.textContent = 'Save';
        }
    });
}
...
async function loadLeaveRequests() {
  ...
  try {
    const mockLeaves = [
        { id: 'lv5', userName: 'Sarah Rose', userDept: 'Marketing', reason: 'Vacation', startDate: '2026-05-10', endDate: '2026-05-15', status: 'pending' },
        { id: 'lv6', userName: 'John Mayer', userDept: 'Marketing', reason: 'Conference', startDate: '2026-05-01', endDate: '2026-05-02', status: 'approved' }
    ];
