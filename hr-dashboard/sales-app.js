...
// ===== MOCK DATA FALLBACKS =====
const mockEmployees = [
    { id: 'emp1', name: 'John Doe', role: 'Sales Executive', dept: 'Sales' },
    { id: 'emp2', name: 'Jane Smith', role: 'Account Manager', dept: 'Sales' },
    { id: 'emp3', name: 'Mike Ross', role: 'Sales Lead', dept: 'Sales' }
];

let mockPipeline = [
    { id: 'p1', name: 'Enterprise Cloud Migration', stage: 'Negotiation', value: '2,500,000', closeDate: '2026-05-15', assigneeName: 'John Doe' },
    { id: 'p2', name: 'SME Security Bundle', stage: 'Proposal', value: '450,000', closeDate: '2026-05-20', assigneeName: 'Jane Smith' },
    { id: 'p3', name: 'Startup Growth Pack', stage: 'Qualification', value: '120,000', closeDate: '2026-06-01', assigneeName: 'Mike Ross' }
];

let mockLeads = [
    { id: 'l1', name: 'Robert Fox', email: 'robert@foxtech.com', assigneeName: 'John Doe', status: 'Warm' },
    { id: 'l2', name: 'Sarah Connor', email: 'sarah@cyberdyne.io', assigneeName: 'Jane Smith', status: 'Hot' },
    { id: 'l3', name: 'Bruce Wayne', email: 'bruce@waynecorp.com', assigneeName: 'Mike Ross', status: 'Warm' }
];

let mockActivities = [
    { user: 'John Doe', action: 'moved deal to Negotiation', time: '10 mins ago', initials: 'JD', color: 'blue' },
    { user: 'Jane Smith', action: 'added a new lead Sarah Connor', time: '1 hour ago', initials: 'JS', color: 'purple' },
    { user: 'Mike Ross', action: 'closed deal Startup Growth Pack', time: '3 hours ago', initials: 'MR', color: 'green' }
];

let mockTasks = [
    { id: 't1', title: 'Call Robert Fox', description: 'Follow up on security proposal', assignedByName: 'Alex Manager', deadline: 'Today', status: 'Pending' },
    { id: 't2', title: 'Prepare Q3 Forecast', description: 'Monthly sales projection', assignedByName: 'Alex Manager', deadline: 'Tomorrow', status: 'In Progress' }
];
...
const pageTitles = {
  ...
  'messages': 'Sales Communications',
  'notifications': 'Sales Inbox',
  'ai-messaging': 'Sales Script Generator',
  'bank': 'Sales Commissions',
  'integration': 'CRM Integrations',
  'settings': 'Settings'
};
...
window.selectNotif = (card, type) => {
    ...
    const contents = {
        'pipeline': {
            ...
            title: 'Large Deal Opportunity',
            from: 'CRM Sync',
            time: '2 minutes ago',
            body: `<p>Hello Alex,</p><p>A new high-value opportunity <b>"Enterprise Cloud Migration"</b> has been moved to the Negotiation stage. Estimated deal value: ₹2,500,000.</p><p>Please review the proposal and prepare for the stakeholder call scheduled for Friday.</p><p>Best regards,<br>Sales Bot</p>`
        },
        'lead': {
            tag: 'Lead Generation',
            ...
            from: 'AI Engine',
            time: '15 minutes ago',
            body: `<p>A potential client from Mumbai has requested a demo for the Enterprise plan. They have a team of 500+ employees.</p><p><b>Action Required:</b> Assign a senior account executive to reach out within the next 30 minutes.</p>`
        },
        'ai': {
            tag: 'AI Insights',
            ...
            time: '2 hours ago',
            body: `<p>The AI Engine detected that sales cycles for the 'Pro' plan have increased by 20% this month.</p><p><b>Recommendation:</b> Offer a limited-time 15% discount for annual commitments to accelerate the closing process.</p>`
        },
        'commission': {
            tag: 'Commission Management',
            title: 'Commission Threshold Reached',
            from: 'Finance Bot',
            time: 'Yesterday',
            body: `<p>Your team has reached 90% of the quarterly sales target. Total commissions accrued: ₹450,000.</p><p>Keep up the momentum to hit the 110% accelerator bonus!</p>`
        }
    };
...
const handleLogout = () => {
  showModal('Logout Confirmation', 'Are you sure you want to end your sales session?', 'log-out', true, async () => {
    try {
      await signOut(auth);
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    try {
      ...
      console.warn('Sales fetch failed, using fallback.');
    }

    if (!data) {
      data = {
        name: localStorage.getItem('userName') || user.displayName || user.email.split('@')[0],
        role: localStorage.getItem('userRole') || 'Sales Manager',
        departmentId: localStorage.getItem('userDept') || 'sales'
      };
    }
...
async function loadBankDetails() {
  try {
    if (document.getElementById('uiBankAcc')) document.getElementById('uiBankAcc').textContent = '**** 4242';
    if (document.getElementById('uiBankName')) document.getElementById('uiBankName').textContent = 'HDFC Bank';
    if (document.getElementById('uiBankRouting')) document.getElementById('uiBankRouting').textContent = 'Routing: 021000021';
    if (document.getElementById('uiBankBalance')) document.getElementById('uiBankBalance').textContent = '₹245,800.00';
  } catch (e) {
...
async function loadDisbursements() {
  ...
  try {
      const mockPayroll = [
          { name: 'John Doe', month: 'April 2026', processedAt: new Date().toISOString(), net: '85,000' },
          { name: 'Jane Smith', month: 'April 2026', processedAt: new Date().toISOString(), net: '72,000' }
      ];
...
async function loadDashboardData() {
  try {
    ...
    try {
      const userDept = localStorage.getItem('userDept') || 'sales';
      ...
      if (auth.currentUser) {
        try {
          const snap = await getDocs(query(collection(db, 'users'), where('departmentId', '==', 'sales')));
...
if (btnGenAI) {
    btnGenAI.addEventListener('click', () => {
        ...
        if (!prompt) {
            if (window.showToast) window.showToast('Input Required', 'Please enter some sales context.', 'warning');
            return;
        }

        btnGenAI.disabled = true;
        btnGenAI.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Analyzing Context...';
        aiResult.style.display = 'block';
        aiResult.innerHTML = '<div class="typing-indicator"><span></span><span></span><span></span></div> AI is crafting your sales script...';

        setTimeout(() => {
            const results = [
                "📞 <b>Cold Call Opener:</b><br>Hi [Prospect Name], I noticed your team is scaling rapidly. Many companies in your sector struggle with [Pain Point]. HRFlow helps teams like yours automate [Feature] to save 10+ hours a week. Do you have 2 minutes to discuss?",
                "📧 <b>Follow-up Email:</b><br>Subject: Quick question about your current HR stack<br>Hi [Prospect Name], I'm following up on our previous conversation. We recently helped a client in your industry increase their efficiency by 30%. I'd love to share the case study with you. Are you available for a brief sync on Tuesday?",
                "🤝 <b>Closing Statement:</b><br>Based on our discussion, it's clear that HRFlow aligns perfectly with your goals for Q4. If we can finalize the agreement by Friday, I can include the premium onboarding package at no extra cost. Shall I send over the contract?"
            ];
            const random = results[Math.floor(Math.random() * results.length)];
            aiResult.innerHTML = `<div style="color:var(--primary); margin-bottom:0.5rem; font-weight:700;"><i class="fa-solid fa-robot"></i> AI SALES SCRIPT:</div>${random}`;
            btnGenAI.disabled = false;
            btnGenAI.innerHTML = 'Regenerate Sales Script';
            if (window.showToast) window.showToast('Script Generated', 'AI has successfully refined your sales approach.', 'success');
        }, 2500);
    });
}

// --- Pipeline & Lead Loading ---
async function loadPipeline() {
    const tbody = document.getElementById('pipelineList');
    if (!tbody) return;
    try {
        tbody.innerHTML = mockPipeline.map(c => `
            <tr>
                <td><b>${c.name}</b></td>
                <td><span class="status-pill" style="background:#eff6ff; color:#3b82f6;">${c.stage}</span></td>
                <td>₹${c.value}</td>
                <td>${c.closeDate || 'No Date'}</td>
    ...
    try {
        tbody.innerHTML = mockLeads.map(l => `
            <tr>
                ...
                <td><span class="status-pill pill-late">${l.status}</span></td>
                <td>
                    <button class="action-btn" title="Contact"><i data-lucide="mail" size="14"></i></button>
                </td>
            </tr>
        `).join('');
        if (window.lucide) lucide.createIcons();
...
// --- Modal Controls ---
window.openCreatePipelineModal = () => { document.getElementById('createPipelineModal').style.display = 'flex'; };
window.closeCreatePipelineModal = () => { document.getElementById('createPipelineModal').style.display = 'none'; };
window.openCreateLeadModal = () => { document.getElementById('createLeadModal').style.display = 'flex'; };
window.closeCreateLeadModal = () => { document.getElementById('createLeadModal').style.display = 'none'; };
...
if (btnSavePipeline) {
    btnSavePipeline.addEventListener('click', async () => {
        ...
        const closeDate = document.getElementById('dealClose').value;
        const assigneeId = document.getElementById('dealAssignee').value;
        const assigneeName = document.getElementById('dealAssignee').options[document.getElementById('dealAssignee').selectedIndex]?.text || '';
        ...
        try {
            btnSavePipeline.disabled = true;
            btnSavePipeline.textContent = 'Saving...';
            // Mock Save
            mockPipeline.push({ id: 'p' + Date.now(), name, stage, value, closeDate, assigneeId, assigneeName });
            closeCreatePipelineModal();
            loadPipeline();
            if (window.showToast) window.showToast('Deal Saved', 'New deal has been added to the pipeline.', 'success');
        } catch (e) { console.error(e); } finally {
            btnSavePipeline.disabled = false;
            btnSavePipeline.textContent = 'Save';
        }
    });
}
...
if (btnSaveLead) {
    btnSaveLead.addEventListener('click', async () => {
        ...
        const assigneeName = document.getElementById('leadAssignee').options[document.getElementById('leadAssignee').selectedIndex]?.text || '';

        if (!name || !email) return;

        try {
            btnSaveLead.disabled = true;
            btnSaveLead.textContent = 'Saving...';
            // Mock Save
            mockLeads.push({ id: 'l' + Date.now(), name, email, assigneeId, assigneeName, status: 'Warm' });
            closeCreateLeadModal();
            loadLeads();
            if (window.showToast) window.showToast('Lead Added', 'New potential customer recorded.', 'success');
        } catch (e) { console.error(e); } finally {
            btnSaveLead.disabled = false;
            btnSaveLead.textContent = 'Save';
        }
    });
}
...
async function loadLeaveRequests() {
  ...
  try {
    const mockLeaves = [
        { id: 'lv1', userName: 'John Doe', userDept: 'Sales', reason: 'Vacation', startDate: '2026-05-10', endDate: '2026-05-15', status: 'pending' },
        { id: 'lv2', userName: 'Jane Smith', userDept: 'Sales', reason: 'Sick Leave', startDate: '2026-05-01', endDate: '2026-05-02', status: 'approved' }
    ];
