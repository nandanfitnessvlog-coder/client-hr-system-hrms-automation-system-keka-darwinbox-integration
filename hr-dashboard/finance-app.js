...
const pageTitles = {
  ...
  'reports': 'Tax & Audit Reports',
  'payroll': 'Payroll Management',
  'messages': 'Team Messages',
  'notifications': 'Fiscal Alerts',
  'ai-messaging': 'Financial AI',
  'bank': 'Corporate Accounts',
  'integration': 'Fiscal Tools',
  'settings': 'Settings'
};
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    try {
      ...
      console.warn('Finance fetch failed, using fallback.');
    }

    if (!data) {
      data = {
        name: localStorage.getItem('userName') || user.displayName || user.email.split('@')[0],
        role: localStorage.getItem('userRole') || 'Finance Manager',
        departmentId: localStorage.getItem('userDept') || 'finance'
      };
    }
...
function updateCharts() {
  ...
  const allCtx = document.getElementById('allocationChart');

  if (!finCtx || !allCtx) return;
  ...
  allocationChartInstance = new Chart(allCtx, {
    type: 'pie',
    data: {
